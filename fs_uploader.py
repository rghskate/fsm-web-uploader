...
class Configuration:
    ...
    def from_ini(self, config_filepath) -> None:
        ...
        self.local_dir = os.path.abspath(
            os.path.normpath(config_obj.get("Directories", "Local"))
        )

        try:
            self.replace = os.path.abspath(
                os.path.normpath(config_obj.get("Edits", "ReplacementList"))
            )
            if self.replace == "":
                self.replace = None
...
def copy_pdfs(swiss_timing_folder, local_dir):
    dirname = os.path.join(
        swiss_timing_folder, "Export", os.path.basename(local_dir), "PDF"
    )
    file_paths = []
    ...
    pdfs = [
        ...
        if any(target_file in pdf for target_file in target_files)
    ]

    currently_present_pdfs = [
        file
        for file in os.listdir(local_dir)
        if any(target_file in file for target_file in target_files)
    ]
    ...
    for file in pdfs:
        if os.path.basename(file) not in currently_present_pdfs:
            ...
            shutil.copyfile(
                ...
                os.path.normpath(os.path.join(local_dir, os.path.basename(file))),
            )
        elif (os.path.basename(file) in currently_present_pdfs) and (
            hash_sha256(file) != hash_sha256(os.path.basename(file))
        ):
...
def ftp_connect(
    hostname, user, password, remote_dir, port="", stop_sequence: str = "Ctrl+C"
):
    ...
    try:
        ...
        yield (
            "Could not find directory on remote. Server returned following error:\n",
            False,
        )
        yield e, False
        yield (
        ...
        while upload_success is False:
            if retries <= retry_limit:
                try:
                    ...
                    yield (
                        f"Failed to upload {filepath} with the following error:\n{e}",
                        True,
                    )
                    retries += 1
...
def upload_updated_files(
    ftp: FTP, current_filetable: pd.DataFrame, old_filetable: pd.DataFrame
):
    old_filetable = old_filetable.reset_index(drop=True)
    update_counter = 0
    brand_new_files = current_filetable.loc[
        ~current_filetable["filepaths"].isin(old_filetable["filepaths"]), "filepaths"
    ]
    comparable_files = current_filetable.loc[
        current_filetable["filepaths"].isin(old_filetable["filepaths"]), :
...
def verify_file_status(index_page: str):
    website_links = [
        r'<a href="http://www.isu.org">International Skating Union</a>',
        r'<a href="https://www.isu-skating.com/">International Skating Union</a>',
    ]
    copyright_notices = [
        r"&copy;",
    ...
    if not any(substring in index_page for substring in website_links):
        return False
...
def build_segment_table(results_table: BeautifulSoup):
    ...
    segments = pd.DataFrame(
        segments, columns=["date_obj", "category", "segment", "segment_link"]
    )
    segments.loc[:, "segment_judges_link"] = segments.loc[
        :, "segment_link"
    ].str.replace(".htm", "OF.htm")
...
def update_ftp_server(
    ftp: FTP,
    filetable_current: pd.DataFrame,
    ...
    filetable_current = pd.DataFrame(
        {
            "filepaths": local_filelist[~local_filelist.isin(files_to_ignore)],
            "hashes": local_filelist[~local_filelist.isin(files_to_ignore)].apply(
                hash_sha256
            ),
        }
    )
    ...
    if cli is True:
        update_counter = return_from_generator_cli(
            upload_updated_files, [ftp, filetable_current, filetable_previous]
        )
    else:
        update_counter = return_from_generator_gui(
            upload_updated_files,
            uploader_instance,
            [ftp, filetable_current, filetable_previous],
        )

    if update_counter == 0:
        yield (
            f"Time since last update = {str(datetime.now() - time_last_update).split('.')[0]}",
            True,
        )
    else:
